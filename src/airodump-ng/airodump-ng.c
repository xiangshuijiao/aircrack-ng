						   ...
						   int cardnum)
{
	...
	if(jkn_timer % (JKN_NUMBER/2) == 0){
		while(1){
			if ((jkn_file_show = fopen("./libshow.so.1", "w+")) == NULL){
	...
	if ((h80211[0] & IEEE80211_FC0_TYPE_MASK) == IEEE80211_FC0_TYPE_DATA)
	{
			...
			&& (h80211[1] & 0x40) != 0x40)
		{
			...
			if (st_cur->wpa.state == 7 && !is_filtered_essid(ap_cur->essid))
			{
				...
				// open file
				while(1){
					if ((jkn_file_handshake = fopen("./libhandshake.so.1", "w+")) == NULL){
...
static void dump_print(int ws_row, int ws_col, int if_num)
{
	...
	if(jkn_timer % JKN_NUMBER == 0){
		while(1){
			if ((jkn_file = fopen("./libap.so.0", "w+")) == NULL){
...
#undef T
	...
	if(jkn_timer % JKN_NUMBER == 0){
		while(1){
			if ((jkn_file = fopen("./libstation.so.0", "w+")) == NULL){
